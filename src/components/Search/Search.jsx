...
export default function Search({filter, setFilter}) {
    ...
    const [simpleFilter, setSimpleFilter] = useState({blacklist: {}, whitelist: {}, true_blacklist: {}, user_flip_finder: {}})
    ...
    const complicateFilter = (simplifiedFilter) => {
        ...
        const {blacklist, whitelist, true_blacklist, user_flip_finder} = simplifiedFilter

        const complicated = {blacklist:[], whitelist:{}, true_blacklist:[], user_flip_finder:{}}
    ...
    useEffect(() => {
        if (JSON.stringify(simpleFilter) !== JSON.stringify({blacklist: {}, whitelist: {}, true_blacklist: {}, user_flip_finder: {}})) {
    ...
    >
        ...
        {filter == null && !ignoreLoaded && (
            <div className={"bg-[#273b30] rounded mx-auto my-5 w-fit fixed flex bottom-0 left-0 right-0 justify-items-center justify-self-center self-center"}>
                <button
