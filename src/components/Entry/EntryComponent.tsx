...
export default function EntryComponent({filterContext, setFilterContext, section, entry, handleClick}: {
    ...
    return (
        <div
            className={"justify-between flex flex-row w-full overflow-x-clip h-16 p-2 bg-accent/70" + (filterContext.focusedEntry?.id === entry.id && " bg-accent/40")}
            ...
            <div className={"flex flex-row"}><img src={`https://sky.shiiyu.moe/item/${entry.item.itemId}`} alt={"Img"}
                ...
                <div className={"flex flex-col"}>
                    ...
                    <p className={"my-auto overflow-x-hidden"}>{entry.stringify()}</p>
                </div>
            </div>
            <button
