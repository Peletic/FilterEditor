...
export default function Home() {
  ...
  useEffect(() => {
    if (
      filter &&
      Object.keys(filter).every((i) => Object.keys(filterStruct).includes(i))
    )
      setSimpleFilter(simplifyFilter(filter));
    else setSimpleFilter(filterStruct);
  }, [filter]);
  ...
  const simplifyFilter = (filter) => {
    ...
    filter.blacklist.forEach((key) => {
      ...
      Object.keys(uniqueAttributes).forEach((attributeName) => {
        if (!itemRef[keyOnly][attributeName])
    ...
    <main className="select-none tracking-tight">
      ...
      <div className="p-5 mx-2 my-5">
        ...
        <div className="relative">
                      ...
                      >
                        ...
                        {suggestion.image ? (
                          <img
                            ...
                            className="w-8 h-8 absolute top-0 right-4"
                          ></img>
                        ) : null}
                      </button>
                    </li>
          ...
          {filter ? <Entries list={filter}></Entries> : null}
        </div>
      </div>
    </main>
  );
